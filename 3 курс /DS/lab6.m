...
ReadGraph[filename_] := Block[{Is,Us,listU,b,fs},
fs = OpenRead[filename];
Is = Read[fs, {Word,Number}][[2]];
Us = Read[fs, {Word,Number}][[2]];
listU = ReadList[fs, Expression, Us];
Print[listU];
b = ReadList[fs,String];
Close[fs];
Return[{Is,Us,listU,b}];
];
